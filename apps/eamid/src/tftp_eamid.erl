...
%%% Description : 
%%%
%%% Created : 13 Feb 2012 by Dmitry Karpov <komm@siphost.su>
%%%-------------------------------------------------------------------

%%%-------------------------------------------------------------------
%%% Usage: 1> tftp:start([{callback, 
%%%			{".cnf", tftp_eamid, [{root_dir, "/tmp"}]}}, 
%%%			{callback, {".*[^cnf]", tftp_file, 
%%%					[{root_dir, "/tmp"}]}}  ]).
%%%-------------------------------------------------------------------
...
do_open(State) when is_record(State, state) ->
    io:format('[DEBUG]: do_open',[]),
%%{ok,Cisco7940}=cisco7940_dtl:render([{number,pp("number",P)}, {sipproxy,"77.77.77.77"}])
	{ok, State#state.options, State#state{fd = <<"komm">> } }
.
...
read(#state{access = read} = State) ->
	io:format('~n[DEBUG]: read, ~w~n',[State#state.filename]),
	{last, <<"komm~nkomm_kommm~n">> , 16}
.

write(Bin, State) -> {error, file_error(eacces)}.
abort(_Code, _Text, #state{fd = Fd, access = Access} = State) -> ok.
